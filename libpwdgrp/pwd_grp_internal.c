...
//	printf("%s(%d) : GETXXKEY_R_FUNC - file=%s\n", __FILE__, __LINE__, GETXXKEY_R_PATHNAME);
...
//		printf("%s(%d) : GETXXKEY_R_FUNC - bb__pgsreader returns=%d\n", __FILE__, __LINE__, rv);
		if (!rv) {
			if (GETXXKEY_R_TEST(resultbuf)) { /* Found key? */
//				printf("%s(%d) : GETXXKEY_R_FUNC - GETXXKEY_R_TEST returns != 0\n", __FILE__, __LINE__);
				*result = resultbuf;
				break;
			}
		} else {
