...
//	printf("%s(%d) : getspnam - name=%s\n", __FILE__, __LINE__, name);
	getspnam_r(name, &resultbuf, buffer, sizeof(buffer), &result);
//	printf("%s(%d) : getspnam - result=%p\n", __FILE__, __LINE__, result);
...
//	printf("%s(%d) : bb__parsespent - EINVAL\n", __FILE__, __LINE__);
...
//	printf("%s(%d) : bb__pgsreader - uid=%d\n", __FILE__, __LINE__, getuid());
	if (buflen < PWD_BUFFER_SIZE) {
		errno = rv;
	} else {
		skip = 0;
...
//				printf("%s(%d) : bb__pgsreader - fgets failed f=%p\n", __FILE__, __LINE__, f);
				if (feof(f)) {
					rv = ENOENT;
//					printf("%s(%d) : bb__pgsreader - feof encountered\n", __FILE__, __LINE__);
				}
				break;
			}
...
//				printf("%s(%d) : bb__pgsreader - line_buff \"%s\"\n", __FILE__, __LINE__, line_buff);
			} else if (line_len + 2 == buflen) { /* line too long */
				++skip;
//				printf("%s(%d) : bb__pgsreader - line too long\n", __FILE__, __LINE__);
				continue;
			}

			if (skip) {
//				printf("%s(%d) : bb__pgsreader - we skip line_buff \"%s\"\n", __FILE__, __LINE__, line_buff);
				--skip;
				continue;
			}
			 ...
			 * whitespace. */
//			printf("%s(%d) : bb__pgsreader - eat line_buff \"%s\"\n", __FILE__, __LINE__, line_buff);
			if (*line_buff && (*line_buff != '#') && !isspace(*line_buff)) {
//				printf("%s(%d) : bb__pgsreader - parserfunc = ", __FILE__, __LINE__);
// 				if (parserfunc == bb__parsepwent) printf("bb__parsepwent\n");
// 				if (parserfunc == bb__parsegrent) printf("bb__parsegrent\n");
// 				if (parserfunc == bb__parsespent) printf("bb__parsespent\n");
...
//				printf("%s(%d) : bb__pgsreader - !parserfunc\n", __FILE__, __LINE__);
			}
		} while (1);
