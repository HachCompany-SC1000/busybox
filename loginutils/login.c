...
// 	printf("%s(%d) : login_main - io redirection\n", __FILE__, __LINE__);
// 	fflush(stdout);
// 	fflush(stderr);
// 	stdout = freopen("/var/log/login.stdout", "w", stdout);
// 	stderr = freopen("/var/log/login.stderr", "w", stderr);
...
//	printf("%s(%d) : login_main - amroot=%d\n", __FILE__, __LINE__, amroot);
...
//		printf("%s(%d) : login_main - username=%s\n", __FILE__, __LINE__, username);

		pw = getpwnam(username);
		if (!pw) {
			safe_strncpy(username, "UNKNOWN", sizeof(username));
//			printf("%s(%d) : login_main - username=UNKNOWN\n", __FILE__, __LINE__);
			goto auth_failed;
		}

//		printf("%s(%d) : login_main - pw_passwd=%s pw_uid=%d\n", __FILE__, __LINE__, pw->pw_passwd?pw->pw_passwd:"no pw", pw->pw_uid);
