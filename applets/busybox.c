...
int main(int argc, char **argv)
{
	...
	applet_name = argv[0];
        //bb_info_msg("run_applet_by_name(%s)", applet_name);
        if (*applet_name == '-')
		applet_name++;
	...
	if (ENABLE_LOCALE_SUPPORT && getpid() != 1)
        ...
        //bb_info_msg("run_applet_by_name(%s, %d, %s, %s, ...)", applet_name, argc, argv[0]?argv[0]:"NULL", argv[1]?argv[1]:"NULL");
	run_applet_by_name(applet_name, argc, argv);
	bb_error_msg_and_die("applet not found - main");
}
...
// XXX: FIXME: this is broken. Why not just use argv[0] ?
	...
	bb_error_msg_and_die("applet not found - busybox_main");
}
